...
def execute_action(action: Dict[str, Any]) -> Union[bool, Tuple[bool, str], List[int]]:
    ...
    try:
        ...
        print(f"{Colors.YELLOW}[CELEIRO] AÇÃO:{Colors.RESET} {description}")
        
        if action_type == "click":
            if len(params) >= 2:
                return click(params[0], params[1])
            else:
                print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Parâmetros insuficientes para ação click")
                return False
            ...
            if len(params) >= 1:
                ...
                print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Parâmetros insuficientes para ação wait")
                return False
            ...
            if len(params) >= 1:
                ...
                print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Parâmetros insuficientes para ação send_keys")
                return False
            ...
            if len(params) >= 2:
                ...
                print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Parâmetros insuficientes para ação searchTemplate")
                return False
            ...
            if len(params) >= 2:
                ...
                for attempt in range(max_attempts):
                    ...
                    if expected_state == current_state_id:
                        print(f"{Colors.GREEN}[CELEIRO] SUCESSO:{Colors.RESET} Estado verificado: {current_state_display} (ID: {current_state_id})")
                        return True
                        
                    print(f"{Colors.YELLOW}[CELEIRO] AGUARDANDO:{Colors.RESET} Estado '{expected_state}', atual: '{current_state_id}' ({current_state_display}) ({attempt+1}/{max_attempts})")
                    wait(0.5)  # Espera 500ms entre as verificações
                
                print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Estado {expected_state} não encontrado após {max_attempts} tentativas")
                return False
            else:
                print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Parâmetros insuficientes para ação verify_state")
                return False
            ...
            if len(params) >= 2:
                ...
                print(f"{Colors.BLUE}[CELEIRO] INFO:{Colors.RESET} Verificando múltiplos estados possíveis: {expected_states}")
                ...
                for attempt in range(max_attempts):
                    ...
                    print(f"{Colors.BLUE}[CELEIRO] INFO:{Colors.RESET} Estado atual: {current_state_display} (ID: {current_state_id}) - Tentativa {attempt+1}/{max_attempts}")
                    
                    # Se o estado for 'unknown', esperamos e tentamos novamente
                    if current_state_id == "unknown":
                        print(f"{Colors.YELLOW}[CELEIRO] AGUARDANDO:{Colors.RESET} Estado em transição (unknown), aguardando...")
                        wait(wait_time)
                        continue
                    
                    # Verificamos se o estado atual é um dos esperados
                    if current_state_id in expected_states:
                        print(f"{Colors.GREEN}[CELEIRO] SUCESSO:{Colors.RESET} Estado atual {current_state_id} está entre os esperados")
                    ...
                    print(f"{Colors.YELLOW}[CELEIRO] AGUARDANDO:{Colors.RESET} Estado atual {current_state_id} não é um dos esperados {expected_states}")
                    wait(wait_time)
                
                # Se chegamos aqui, não encontramos um estado válido após todas as tentativas
                print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Estado válido não encontrado após {max_attempts} tentativas")
                return (False, None)
            else:
                print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Parâmetros insuficientes para ação check_multiple_states")
                return False
            ...
            if len(params) >= 1:
                ...
                if empty_boxes and len(empty_boxes) > 0:
                    print(f"{Colors.GREEN}[CELEIRO] SUCESSO:{Colors.RESET} Encontradas {len(empty_boxes)} caixas vazias")
                    return empty_boxes
                else:
                    print(f"{Colors.YELLOW}[CELEIRO] AVISO:{Colors.RESET} Nenhuma caixa vazia encontrada")
                    return []
            else:
                print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Parâmetros insuficientes para ação scan_empty_boxes")
                return False
        ...
        print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Falha ao executar ação {action.get('type', 'desconhecida')}: {e}")
        return False
    ...
    try:
        ...
        if use_mask:
            ...
            if not os.path.exists(mask_path):
                ...
                if not os.path.exists(mask_path):
                    print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Máscara não encontrada para {template_path}")
                    return False
            
            # Configura o MaskedTemplateMatcher
            template_matcher = MaskedTemplateMatcher(default_threshold=threshold, verbose=False)
            print(f"{Colors.BLUE}[CELEIRO] INFO:{Colors.RESET} Usando template matcher com máscara")
        else:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            print(f"{Colors.BLUE}[CELEIRO] CAPTURANDO:{Colors.RESET} Nova screenshot para busca de template (tentativa {attempt}/{max_attempts})")
            screenshot = screenshotter.take_screenshot(use_pil=False)
            ...
            if result and result.get('found', False):
                ...
                if click_result:
                    ...
                    print(f"{Colors.YELLOW}[CELEIRO] AGUARDANDO:{Colors.RESET} Verificando se o estado mudou após o clique...")
                    ...
                    if current_state_id == "inside_shop":
                        print(f"{Colors.GREEN}[CELEIRO] SUCESSO:{Colors.RESET} Loja aberta com sucesso!")
                        return True
                    
                    # Se não mudou para inside_shop, tenta deslocamentos
                    print(f"{Colors.YELLOW}[CELEIRO] AVISO:{Colors.RESET} Clique na posição original não abriu a loja. Tentando deslocamentos...")
                else:
                ...
                for i, (dx, dy) in enumerate(offsets):
                    ...
                    if current_state_before_click == "inside_shop":
                        print(f"{Colors.GREEN}[CELEIRO] SUCESSO:{Colors.RESET} Loja já está aberta, não é necessário continuar tentando deslocamentos")
                        return True
                        
                    new_x, new_y = x + dx, y + dy
                    print(f"{Colors.YELLOW}[CELEIRO] TENTATIVA {i+1}/{len(offsets)}:{Colors.RESET} Clicando em posição deslocada ({new_x}, {new_y})")
                    
                    if click(new_x, new_y):
                        # Aguarda por 1.4 segundos para ver se o estado muda (tempo ajustado conforme kit_terra)
                        print(f"{Colors.YELLOW}[CELEIRO] AGUARDANDO:{Colors.RESET} Verificando por 1.4s se o estado mudou após clique com deslocamento...")
                        wait(1.4)  # Aguarda 1.4 segundos para o estado mudar após cliques com deslocamento
                        current_state_id = get_current_state_id()
                        if current_state_id == "inside_shop":
                            print(f"{Colors.GREEN}[CELEIRO] SUCESSO:{Colors.RESET} Loja aberta com sucesso após tentativa com deslocamento!")
                            return True
                
                print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Todas as tentativas de clique falharam em abrir a loja")
                return False
            else:
...
def run() -> bool:
    ...
    try:
        ...
        print(f"{Colors.GREEN}[CELEIRO] \u2714\ufe0f{Colors.RESET} Registro de callback de estado concluído.")
        ...
        kit_config = config["kit_celeiro"]
        
        # Obtém o estado atual do jogo
        current_state_id = get_current_state_id()
        current_state_display = get_current_state()
        print(f"{Colors.BLUE}[CELEIRO] INFO:{Colors.RESET} Estado atual: {current_state_display} (ID: {current_state_id})")
        ...
        while True:
            ...
            if current_state_id not in kit_config["states"]:
                print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Estado {current_state_id} não está configurado para o Kit Celeiro")
                unregister_state_callback(on_state_change_during_execution)
                return False
                
            # Se o estado atual é "inside_shop", significa que atingimos o objetivo de navegação
            # e podemos prosseguir com as ações específicas do Kit Celeiro
            if current_state_id == "inside_shop":
                print(f"{Colors.GREEN}[CELEIRO] SUCESSO:{Colors.RESET} Chegamos à loja, prosseguindo com as ações do Kit Celeiro")
            ...
            print(f"{Colors.BLUE}[CELEIRO] INFO:{Colors.RESET} Executando {len(actions)} ações para o estado '{current_state_display}'")
            
            for i, action in enumerate(actions):
                # Executa a ação
                print(f"{Colors.YELLOW}[CELEIRO] AÇÃO {i+1}/{len(actions)}:{Colors.RESET} {action.get('description', '')}")
                ...
                if isinstance(result, tuple) and len(result) >= 2:
                    ...
                    if not success:
                        print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Falha ao executar ação: {action.get('description', '')}")
                        unregister_state_callback(on_state_change_during_execution)
                        return False
                    print(f"{Colors.BLUE}[CELEIRO] INFO:{Colors.RESET} Redirecionando para o estado: {next_state}")
            ...
            if state_changed_flag:
                print(f"{Colors.BLUE}[CELEIRO] INFO:{Colors.RESET} Estado mudou para: {get_current_state()} (ID: {last_detected_state_id})")
                state_changed_flag = False
                
                # Se for um tutorial pop-up, reinicia o processo após lidar com ele
                if pending_restart:
                    pending_restart = False
                    print(f"{Colors.YELLOW}[CELEIRO] REINICIANDO:{Colors.RESET} Após lidar com pop-up de tutorial")
                    continue
                
                # Se estamos na loja, prosseguimos para a próxima fase
                if last_detected_state_id == "inside_shop":
                    print(f"{Colors.GREEN}[CELEIRO] SUCESSO:{Colors.RESET} Chegamos à loja após mudança de estado")
                    break
            ...
            if next_state:
                ...
                if current_state_id == "inside_shop":
                    print(f"{Colors.GREEN}[CELEIRO] SUCESSO:{Colors.RESET} Já estamos na loja")
                    break
                # Se não temos um próximo estado e concluímos todas as ações, espera mudança auto. de estado
                print(f"{Colors.BLUE}[CELEIRO] INFO:{Colors.RESET} Aguardando mudança de estado após ações")
                wait(1.0)  # Espera um segundo antes de verificar novamente
        
        # Agora que estamos na loja, executamos as ações específicas do Kit Celeiro
        print(f"{Colors.BLUE}[CELEIRO] INFO:{Colors.RESET} Executando ações específicas da loja")
        ...
        for i, action in enumerate(inside_shop_actions):
            # Executa a ação
            print(f"{Colors.YELLOW}[CELEIRO] AÇÃO {i+1}/{len(inside_shop_actions)}:{Colors.RESET} {action.get('description', '')}")
            ...
            if action.get("type") == "scan_empty_boxes":
                ...
                if isinstance(result, list):
                    empty_boxes = result
                    print(f"{Colors.GREEN}[CELEIRO] SUCESSO:{Colors.RESET} Identificadas {len(empty_boxes)} caixas vazias")
                    
                    # Carrega a configuração de itens do kit
                    items_config = load_items_config()
                    if items_config:
                        print(f"{Colors.BLUE}[CELEIRO] INFO:{Colors.RESET} Iniciando preenchimento de caixas com Kit Celeiro")
                        
                        # Usa o framework para preencher as caixas com os itens do kit
                        kit_result = process_kit(items_config, empty_boxes)
                        if kit_result:
                            print(f"{Colors.GREEN}[CELEIRO] SUCESSO:{Colors.RESET} Kit Celeiro aplicado com sucesso!")
                        else:
                            print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Falha ao aplicar Kit Celeiro")
                            return False  # Retorna False se o processamento do kit falhar
                    else:
                        print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Não foi possível carregar a configuração de itens do Kit Celeiro")
                else:
                    print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Falha ao identificar caixas vazias")
            else:
        ...
        print(f"\n{Colors.GREEN}[CELEIRO] OPERAÇÃO CONCLUÍDA:{Colors.RESET} Kit Celeiro executado com sucesso!\n")
        return True
        
    except Exception as e:
        print(f"{Colors.RED}[CELEIRO] ERRO:{Colors.RESET} Falha ao executar Kit Celeiro: {e}")
